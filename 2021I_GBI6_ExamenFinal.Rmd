...
# Escriba aquí su interpretación 
 ##En la tabla se puede visiaulizar estadisticamente la descrpcion del cancer de MAMA y su nivel de expresion de los diferentes genes por lo que a partir de diferentes variables, tales como los tipos de cancers, las etiquetass, el recortado, la media, el rango, el sesgar de los pacientes o muestras obtenidas en el cual se observa que existe un bajo rango de cada tipo.
```
...
##En esta grafica se puede visualizar los tipos de ancer en el cual indica el tipo de data set o el resto de genes en los diferentes datasets en l cual es la GATA3 (genes de expresio) con respecto FN, donde existe mayor concentracion entre -2 y -4 de los genes en ese tipo de cancer de MAMA.
```
